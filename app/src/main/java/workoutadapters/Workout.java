...
import database.ExerciseDataCursorLoader;
import database.SetDataCursorLoader;
...
public class Workout extends Activity implements  OnItemClickListener, OnTouchListener, LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onSaveInstanceState(Bundle outState) {
        if (outState == null) throw  new AssertionError("outState is null when onSaveInstanceState of workout");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        exerciseLogAdapter = new WorkoutDataAdapter(this, exCursor, Subject.EXERCISES);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onOptionsItemSelected " + item.getItemId());
    ...
    public void initiateListUpdate( Subject trigSubj, TriggerEvent trigEvent ) {
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: initiateListUpdate subj: " + trigSubj.toString() + " caused by: " + trigEvent.toString());
        int subject = -1;
        ...
        if ( trigEvent == TriggerEvent.NOTEADD && trigSubj != Subject.EXERCISES && trigSubj != Subject.SETS ) {
            if ( DEBUG_FLAG ) Log.v( APP_NAME, "Workout :: initiateListUpdate : incoming parameters are messed" );
            return;
        }
        ...
        switch ( trigEvent ) {
            ...
            default:
                Log.e( APP_NAME, "Workout :: initiateListUpdate : unexpected trigger event: "+trigEvent.toString() );
        }
    }

    public void onBackButtonPressed(View v) {

        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onBackButtonPressed()");
    ...
    public void onAddButtonPressed(View v) {
        ...
        if (exerciseTextView.getVisibility() == View.VISIBLE) {
            ...
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onAddButtonPressed(). Exercise in edit text: \"" + incomingName + "\" Amount of exercises before btn pressed: "+exerciseLogAdapter.getCount() );
            ...
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onAddButtonPressed() Reps: " + repsEdit.getText() + " Weight: " + weightEdit.getText() + " Curr ex idx: " + exerciseLogAdapter.getIdxOfCurrent());
            ...
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onAddButtonPressed(). current exercise: "+ exerciseLogAdapter.getIdxOfCurrent());
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onItemClick :: position: " + position + " id: "+id + " view ID: "+view.getId() );
        ...
        switch (view.getId()) {
            case R.id.exercise_entry_container:
                ...
                if (exerciseLogAdapter.getIdxOfCurrent() == position) {
                    if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onItemClick :: same item clicked, nothing to do.");
                } else {
    ...
    public boolean syncListPositions( Subject baseSubject ) {
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: syncListPositions :: started. base subject: " + baseSubject.toString() );
        ...
        if ( baseSubject  == Subject.EXERCISES ) {
            ...
            Log.e(APP_NAME, "Workout :: syncListPositions :: unexpected subject: " + baseSubject.toString() );
            return false;
        }

        if ( baseAdapter.getIdxOfCurrent() == -1  ) {
            Log.d(APP_NAME, "Workout :: syncListPositions :: doing nothing since current for one of the adapters is not set" );
            return false;
        }
        ...
        for ( ; !trgtCs.isAfterLast() ; trgtCs.moveToNext() ) {
            ...
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: syncListPositions :: sourceId: "+sourceId+" targetId:"+trgtCs.getInt( targetKeyFieldIdx ) );

            //may be several appropriate items in a sequence
            if ( trgtCs.getInt( targetKeyFieldIdx ) == sourceId ){
                targetPos = trgtCs.getPosition();
                if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: syncListPositions :: intermediate position of target: "+targetPos );
                continue;
            }
        ...
        if (targetPos != -1) {
            ...
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: syncListPositions :: resulting target found. SourceId: "+sourceId+" Target moved from "+initialTargetPos+ " to "+targetPos );
            return true;
        } else {
            trgtCs.moveToPosition( initialTargetPos );

            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: syncListPositions :: target not found ( sourceId: "+sourceId+")" );
            return false;
        }
    ...
    * will change current adapter to appropriate one
    ...
    public void onNotesTap(View v) {
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onNotesTap");
        String headText;
        ...
        if ( subjAdapter.getCount() == 0 || subjAdapter.getIdxOfCurrent() == -1 ) {
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onNotesTap : doing nothing since no source for note exist");
            return;
        }

        String note =  subjAdapter.getNoteForCurrent();
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onNotesTap : subj: "+subjAdapter.getSubject().toString()+" current: "+subjAdapter.getIdxOfCurrent()+ " note: " + note);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onActivityResult");
        ...
        if (resultCode == RESULT_OK) {
            ...
            if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onActivityResult once got item");

            switch (requestCode) {

                case 1: //exercise
                    if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onActivityResult : gonna insert note \"" + note + "\" for ex  \"" + exerciseLogAdapter.getNameForCurrent()+"\"" );
    ...
    public void onContextButtonPressed(View contextualActionButton) {
        ...
        if ( DEBUG_FLAG ) Log.v(APP_NAME, "Workout :: onContextButtonPressed :: active subject: " + currSubj.toString() );
        ...
        switch ( currSubj ) {
            ...
            default:
                Log.e(APP_NAME, "Workout :: onContextButtonPressed. Weird state met");
                return;
        }
        ...
        switch (contextualActionButton.getId()) {
            ...
            default:
                Log.e(APP_NAME, "Workout :: onContextButtonPressed. handler for passed view is missing");
        }
    }
        ...
        switch ( loader.getId() ) {
            ...
            case SETS:
                ...
                if ( setsLogAdapter == null ) {
                    setsLv.setAdapter( setsLogAdapter = new WorkoutDataAdapter(this, data, Subject.SETS) );
                } else {
